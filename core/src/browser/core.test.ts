...
import { getMiaodaDataFolderPath } from './core'
...
describe('test core apis', () => {
  ...
  it('should get jan data folder path', async () => {
    globalThis.core = {
      api: {
        getMiaodaDataFolderPath: vi.fn().mockResolvedValue('/path/to/miaoda/data'),
      },
    }
    const result = await getMiaodaDataFolderPath()
    expect(globalThis.core.api.getMiaodaDataFolderPath).toHaveBeenCalled()
    expect(result).toBe('/path/to/miaoda/data')
  })
})
