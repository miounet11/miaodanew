...
const mockServiceHub = {
  ...
  app: () => ({
    ...
    relocateMiaodaDataFolder: vi.fn().mockResolvedValue(undefined),
    getMiaodaDataFolder: vi.fn().mockResolvedValue('/mock/miaoda/data'),
  }),
  analytic: () => ({
    track: vi.fn(),
    identify: vi.fn(),
...
// Mock globalThis.core.api for @miaoda/core functions // cspell: disable-line
;(globalThis as Record<string, unknown>).core = {
  api: {
    getMiaodaDataFolderPath: vi.fn().mockResolvedValue('/mock/miaoda/data'),
    openFileExplorer: vi.fn().mockResolvedValue(undefined),
    joinPath: vi.fn((...paths: string[]) => paths.join('/')),
  }
}

// Mock globalThis.fs for @miaoda/core fs functions // cspell: disable-line
